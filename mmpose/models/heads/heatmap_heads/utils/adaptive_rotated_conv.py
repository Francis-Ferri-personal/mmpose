...
# Recovered from: https://github.com/LeapLabTHU/ARC/blob/4825acd8d5777d668ea4fb3b4d24aa53399d79ff/mmdet/models/backbones/modules/adaptive_rotated_conv.py

__all__ = ['AdaptiveRotatedConv2d']
# Thetas are rotation angles
def _get_rotation_matrix(thetas):
    bs, g = thetas.shape # batch size and number of groups (kernels)
    ...
    # Construct elementary combinations that are then used to fill the entries of the rotation matrix.
    """
        These combinations (a, b, c) are heuristics designed to approximate a smooth, differentiable rotation between the 9 kernel values. They are not exact geometric rotations, but rather constructed so that they interpolate between kernel positions when rotating.
    """
    a = x - y
    b = x * y
    c = x + y

    # Each row of the matrix is a vector that defines how to rotate a specific position of the kernel.
    # The first rotated pixel (position 0 of the new kernel) is composed of times the original pixel 0 + (1-a) times the original pixel 1.
...
def batch_rotate_multiweight(weights, lambdas, thetas):
    ...
    rotation_matrix = _get_rotation_matrix(thetas) # generates differentiable rotation matrices, one by theta.
    ...
    rotation_matrix = torch.mul(rotation_matrix, lambdas) # Scales each rotation matrix with its corresponding lambda (intensity control).
    ...
    weights = weights.permute(0, 3, 4, 1, 2) # we make this reshape because it is much more faster & efficient
...
# TODO:
"""
If you want MMPose to automatically initialize your modules, you can:

Register your class (@MODELS.register_module()).

Define init_cfg for it.

Or call self.init_weights() and use mmcv.cnn.initialize.
"""
