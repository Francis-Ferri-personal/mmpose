...
model = dict(
    ...
    head=dict(
        ...
        num_keypoints=19,
        gfd_channels=32,
        att_channels=8, # Note: maybe 16 is too few
        gfd_inter_channels=32, # NOTE: Usual implementation =  gfd_inter_channels = gfd_channels for coord attention
        use_bbox=False,
        rel_pos_enc_start=False, # Relative positional encoding start
        similarity_type=None, # 'Cosine', 'Cross-correlation', None
        coord_att_type='Default', # 'Default', 'Concatenated' 'PreFiLM', 'PreFiLM-Gated', None
        rel_pos_enc_end=False, # Relative positional encoding end
